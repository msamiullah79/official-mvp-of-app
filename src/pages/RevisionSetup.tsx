...
const RevisionSetup = () => {
  ...
  const handleStart = () => {
    ...
    setError("");
    navigate(
      `/practice/${yearSlug}/${moduleSlug}/revision?mode=timed&count=${selected}&randomize=true&timer=${selected}`
    );
  };
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      >
        <h1 className="text-xl font-bold text-foreground mb-1">Module Revision Setup</h1>
        <p className="text-sm text-muted-foreground mb-6">
          {mod?.name || "Module"} · Mixed MCQs across all subjects
        </p>

        <div className="mb-6">
          <p className="text-sm text-muted-foreground mb-1">
            Total questions available: <span className="font-semibold text-foreground">50</span>
          </p>
